...
must_haves:
  ...
  artifacts:
    ...
    - path: "js/input.js"
      ...
      contains: "keyup"
    - path: "js/main.js"
      provides: "Player instantiation, physics update in game loop, jump wiring"
...
**js/player.js** -- Create new file. This is the player entity module.
...
1. Exports createPlayer, updatePlayer, handleJumpPress, handleJumpRelease
2. createPlayer returns object with all required state fields (x, y, velocityX, velocityY, isGrounded, jumpsRemaining, coyoteTimer, squashX, squashY, animState, animFrame, animTime)
3. updatePlayer applies gravity with FALL_GRAVITY_MULT when velocityY > 0
4. updatePlayer caps player position to ground (GROUND_Y - PLAYER_HEIGHT)
5. handleJumpPress checks coyoteTimer AND jumpsRemaining atomically
6. handleJumpRelease multiplies velocityY by JUMP_CUT_MULTIPLIER when velocityY < 0
7. All physics values read from CONFIG (not hardcoded)
...
3. touchend listener exists with passive: false
4. Touch hold duration logic compares against CONFIG.TOUCH_JUMP_SHORT_MS
...
4. TOUCH_JUMP_SHORT_MS exists
  ...
  <done>player.js entity module created with full jump physics (double jump, variable height, coyote time, asymmetric gravity, horizontal movement); input.js upgraded with keyup and touchend tracking; config.js extended with all player constants</done>
</task>
...
6. **Add left/right input handling:**
   - In the setupInput call or separately, add keydown listeners for ArrowLeft and ArrowRight that set `player.velocityX = -CONFIG.PLAYER_MOVE_SPEED` or `CONFIG.PLAYER_MOVE_SPEED`.
   - Add keyup listeners for ArrowLeft and ArrowRight that set `player.velocityX = 0` (stop moving when key released).
   - IMPORTANT: This can be done by extending input.js OR by adding key handlers directly in main.js. The simpler approach is to add a `setupMovement(canvas, player)` section in main.js that adds keydown/keyup listeners for ArrowLeft/ArrowRight on the canvas. This keeps input.js focused on the action callback pattern and avoids overcomplicating it.
...
IMPORTANT: Do NOT break the existing start screen rendering. The READY state still shows the title and pulsing text. The RUNNING state now shows the player rectangle on the ground instead of the ghost text.
  ...
  <verify>
Serve the project: `npx http-server C:/development/halbritter-lab/KidneyQuest -p 8080 -c-1`
Open http://localhost:8080 in browser.

Check:
1. Start screen still works (KidneyQuest title, pulsing text, ground line)
2. Press Space -- game transitions to RUNNING, green rectangle appears standing on the ground
3. Press Space again -- rectangle jumps with a smooth upward arc, then falls back down faster than it rose (asymmetric gravity)
4. Quick tap Space -- small hop (variable jump height working)
5. Hold Space longer -- full-height jump
6. While airborne from first jump, press Space -- double jump fires (slightly weaker arc)
7. Cannot triple-jump (pressing Space while double-jump is active does nothing)
8. After landing, can immediately jump again
9. Press ArrowLeft -- rectangle moves left, soft bounces at zone boundary
10. Press ArrowRight -- rectangle moves right, soft bounces at zone boundary
11. Rectangle squashes briefly on landing (visible width stretch, height compress), then springs back
12. No console errors
13. Open console, type `CONFIG.GRAVITY = 900` -- jump arc changes (lower gravity = floatier)
14. Type `CONFIG.JUMP_VELOCITY = -800` -- jump goes higher
  </verify>
  <done>Player rectangle is playable in the game loop with full jump physics (double jump, variable height, coyote time, asymmetric gravity, squash/stretch landing, horizontal movement)</done>
</task>
